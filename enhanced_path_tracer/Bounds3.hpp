                                ...
                                const std::array<int, 3>& dirIsPos) const
{
    ...
    //find tEnter
    for (int i = 0;i < 3;i++) {
        float tMin = ((dirIsPos[i] ? pMin[i] : pMax[i]) - ray.origin[i]) * invDir[i];
        tEnter = std::max(tEnter, tMin);
    }

    //find tExit
    for (int i = 0;i < 3;i++) {
        float tMax = ((dirIsPos[i] ? pMax[i] : pMin[i]) - ray.origin[i]) * invDir[i];
        tExit = std::min(tExit, tMax);
    }

    //If tEnter=tExit, the ray does not enter the bounding box
    if (tEnter <= tExit + 1e-6f  && tExit>0)
        return true;
    else return false;
